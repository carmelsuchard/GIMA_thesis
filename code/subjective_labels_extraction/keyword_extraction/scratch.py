import pandas as pd
subject_col = "subject"

GROUND_TRUTH_CSV = r"C:\Users\5298954\Documents\Github_Repos\GIMA_thesis\code\data_cleaning\annotations_summary_manually_edited.xlsx"
PREDICTIONS_CSV = r"C:\Users\5298954\Documents\Github_Repos\GIMA_thesis\code\subjective_labels_extraction\keyword_extraction\keyword_extraction.csv"

df_ref = pd.read_excel(GROUND_TRUTH_CSV, usecols=["Thesis_File", subject_col], engine="openpyxl")
# df_pred = pd.read_csv(PREDICTIONS_CSV, usecols=["thesisID", "subject_predicted"], encoding="latin-1")

print(df_ref)